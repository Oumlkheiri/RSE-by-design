...
export default function Home() {
  ...
  return (
    <>
    <div className="min-h-[300vh] md:min-h-[300vh] bg-[#0d0905] overflow-x-hidden">
      {/* hero section */}
      <motion.section 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <img src="/images/bg-sec.png" alt="background" className="w-full h-full object-cover object-center" />
        ...
        {mounted && bubbles.map((bubble, index) => (
          ...
          >
            <motion.div 
              ...
              className="co2 flex flex-col border border-white/30 rounded-full bg-[${bubble.color}] backdrop-blur-md text-white text-center items-center justify-center w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]"
            >
      ...
      >
        <h1 className="text-4xl sm:text-6xl md:text-[8rem] lg:text-[12rem] xl:text-[16rem] font-bold bg-gradient-to-b from-white to-white/20 bg-clip-text text-transparent leading-tight">RENAISSANCE</h1>
        {/* png image */}
        <img src="/images/plant.png" alt="background" className="w-[8rem] sm:w-[12rem] md:w-[18rem] lg:w-[20rem] absolute translate-y-[-4rem] sm:translate-y-[-6rem] md:translate-y-[-11rem] lg:translate-y-[-13.1rem] translate-x-[0rem] md:translate-x-[-1.1rem]" />
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold translate-y-[-0.5rem] sm:translate-y-[-1rem] md:translate-y-[-1.5rem] lg:translate-y-[-2rem] max-w-xs sm:max-w-md md:max-w-none px-2">De chaque petite graine naît une nouvelle vie.</p>
        <img src="/images/racine.png" alt="text" className="w-[12rem] sm:w-[15rem] md:w-[18rem] lg:w-[20rem] absolute translate-y-[15rem] sm:translate-y-[20rem] md:translate-y-[60rem] lg:translate-y-[80rem] opacity-20 translate-x-[-0.5rem] sm:translate-x-[-1.1rem] blur-[1px]"/>
      </motion.div>
      ...
      <div className="card absolute top-[60vh] sm:top-[70vh] left-0 w-full h-full z-10">
        {points.map((point, index) => (
          <motion.div 
            ...
            className={`flex flex-col items-start justify-center absolute top-1/4 sm:top-1/3 w-[90%] sm:w-[85%] md:w-[35%] lg:w-[30%] left-1/2 md:left-0 -translate-x-1/2 md:translate-x-0 ml-0 md:ml-[3rem] lg:ml-[5rem] px-4 sm:px-6 md:px-0`}
            style={{ 
              transform: isMobile 
                ? `translate(-50%, ${index * 50}vh)` 
          ...
          >
            ...
            <motion.p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-300 leading-relaxed sm:leading-relaxed text-left">
